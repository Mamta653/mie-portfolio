...
export default function ProjectCard({
  ...
  const statusConfig: Record<string, { color: string; label: string }> = {
    ...
    fullstack: { color: 'bg-emerald-400', label: 'Full Stack' },
    'coming-soon': { color: 'bg-slate-500', label: 'Coming Soon' },
  };
  const config = statusConfig[status] ?? { color: 'bg-slate-500', label: 'Coming Soon' };
      ...
      <div className='relative p-8 h-full flex flex-col'>
        ...
        {tags.length > 0 && (
          <div className='flex flex-wrap gap-2 mt-6'>
            {tags.map((tag) => (
              <span key={tag} className='px-3 py-1 text-xs font-medium tracking-wide text-emerald-300/90 bg-emerald-500/10 border border-emerald-400/20 rounded-full'>
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className='flex items-center justify-between mt-6'>
          <div className='flex items-center gap-2'>
            ...
            <span className='text-xs text-slate-500 tracking-wide'>{config.label}</span>
          </div>
          <div className='flex items-center gap-3'>
            {github && status !== 'coming-soon' && (
              <a href={github} target='_blank' rel='noopener noreferrer' className='flex items-center gap-1 text-xs text-slate-400 hover:text-emerald-400 transition-colors duration-300'>
                <Github size={14} />
                GitHub
              </a>
            )}
            {liveUrl && (
              <a href={liveUrl} target='_blank' rel='noopener noreferrer' className='flex items-center gap-1 text-xs text-emerald-400 hover:text-emerald-300 transition-colors duration-300'>
